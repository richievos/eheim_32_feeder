...
void setupMQTT(MqttBroker& mqttBroker, MqttClient& mqttClient, const std::shared_ptr<TopicProcessorMap> topProcessor) {
...
void loopMQTT(MqttBroker& mqttBroker, MqttClient& mqttClient) {
    mqttBroker.loop();
    mqttClient.loop();
}
}  // namespace mqtt
